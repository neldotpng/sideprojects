...
const useScroller = (containerRef, reactive = false) => {
  const { size } = useThree();
  ...
  const getScrollData = (containerRef) => {
    ...
    const sectionProgress = (scrollTop + sectionOffset - sectionData.top) / (sectionData.height - sectionHeightOffset);
  ...
  const scrollData = useRef(getScrollData(containerRef));

  const updateScrollData = (containerRef, prevState) => {
    const { scrollBottom, progress, sectionProgress, section, sectionInfo } = getScrollData(containerRef);
  ...
  useFrame(() => {
    ...
    updateScrollData(containerRef, scrollData.current);
