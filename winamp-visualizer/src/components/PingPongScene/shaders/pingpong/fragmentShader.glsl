...
void main() {
  ...
  vec2 p = vec2(pow(uBass, 2.), pow(uBass, 2.)) * 0.3;
  ...
  z -= .5;
  z = vec2(pow(length(z), uEnergy * 1.5));
  z = smoothstep(0.1, e, z);
  // z = mod(z, 5.);
  // z = smoothstep(0., 1., z);
  ...
  color = mix(
    ...
    1. - palette( z.x + z.y, vec3(0.5,0.5,0.5),vec3(0.5,0.5,0.5),vec3(1.0,1.0,1.0),vec3(0.0,0.10,0.20)),
    0.3
  );
  color *= 0.99; // fade
