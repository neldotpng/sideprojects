...
void main() {
  ...
  vUv = uv;
  vClip = worldPosition.xy / worldPosition.w * 0.5 + 0.5;

  vec4 texture = texture2D(uTexture, vClip.xy);

  float x = remap(texture.r, -5., 5., -.75, .75);
  float y = remap(texture.g, -5., 5., -.75, .75);
